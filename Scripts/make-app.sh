#!/bin/bash
#
# Builds the ClaudeUsageBar.app bundle.
#
# Usage:
...
# refreshes /Applications/ClaudeUsageBar.app, and opens the new build so the
# menu bar item comes back with your latest changes.
#
set -euo pipefail
cd "$(dirname "$0")/.."
...
quit_running() {
    if pgrep -f "${APP_NAME}$" >/dev/null 2>&1 || pgrep -f "${APP_NAME}.app" >/dev/null 2>&1; then
        echo "› Quitting running copy…"
        osascript -e "quit app \"${APP_NAME}\"" 2>/dev/null || pkill -f "${APP_NAME}" || true
